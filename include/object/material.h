...
class Material : public Asset {
 public:
  ...
  template <class T>
  bool createUniform(const std::string& name, const T& value = T(),
  ...
  template <class T>
  bool addUniform(const Uniform<T>& u, std::vector<Uniform<T>>& uniforms) {
    unsigned int i = 0;
    bool result = true;
    while (i < uniforms.size() && !result) {
      ...
      if (uniform.getName() == u.getName()) {
        result = false;
        std::cerr << "Uniform with name " << u.getName() << " already exist"
                  << std::endl;
      }
      ++i;
    }
  ...
  bool addTexture(const std::string& name, std::shared_ptr<DDTexture> texture);
  ...
  bool setUniform(const std::string& name, unsigned int value);
  ...
  template <class T>
  bool getUniform(const std::string& name, T& value,
  ...
  bool getUniform(const std::string& name, unsigned int& value) const;
  ...
  bool getTexture(const std::string& name,
                  std::shared_ptr<DDTexture>& texture) const;
  bool getCubeTexture(const std::string& name,
                      std::shared_ptr<CubeTexture>& texture) const;

  [[nodiscard]] const std::vector<Uniform<float>>& get1fUniforms() const;
  [[nodiscard]] const std::vector<Uniform<int>>& get1iUniforms() const;
  [[nodiscard]] const std::vector<Uniform<unsigned int>>& get1uiUniforms()
      const;
  [[nodiscard]] const std::vector<Uniform<bool>>& get1bUniforms() const;
  [[nodiscard]] const std::vector<Uniform<glm::vec3>>& get3fUniforms() const;
  [[nodiscard]] const std::vector<Uniform<glm::vec4>>& get4fUniforms() const;
  [[nodiscard]] const std::vector<Uniform<glm::mat3>>& get3x3fUniforms() const;
  [[nodiscard]] const std::vector<Uniform<glm::mat4>>& get4x4fUniforms() const;

  [[nodiscard]] const std::map<std::string, std::shared_ptr<DDTexture>>&
  getTextures() const;
  [[nodiscard]] const std::map<std::string, std::shared_ptr<CubeTexture>>&
  getCubeTextures() const;
 ...
 private:
  ...
  std::map<std::string, std::shared_ptr<DDTexture>> _textures;
  std::map<std::string, std::shared_ptr<CubeTexture>> _cubeTextures;
};
