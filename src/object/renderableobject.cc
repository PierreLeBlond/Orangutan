...
void RenderableObject::initVertexArrayObject() {
  if (_shaderMaterial.getShaderStrategy()->get_vertex_attribute() > -1)
    _vao.attribBuffer(
        _idOfPositionArray,
        _shaderMaterial.getShaderStrategy()->get_vertex_attribute(), 3,
        O_FLOAT);
  if (_shaderMaterial.getShaderStrategy()->get_normal_attribute() > -1)
    _vao.attribBuffer(
        _idOfNormalArray,
        _shaderMaterial.getShaderStrategy()->get_normal_attribute(), 3,
        O_FLOAT);
  if (_shaderMaterial.getShaderStrategy()->get_texture_coordinate_attribute() >
      -1)
    _vao.attribBuffer(
        _idOfTexCoordArray,
        _shaderMaterial.getShaderStrategy()->get_texture_coordinate_attribute(),
        2, O_FLOAT);
}
...
void RenderableObject::fillInVBO() {
  ...
  if (_shaderMaterial.getShaderStrategy()->get_texture_coordinate_attribute() >
      -1)
    _vao.fillBuffer(_idOfTexCoordArray, _mesh->getNumberOfUvs() * sizeof(float),
                    _mesh->getUvs().data());
...
void RenderableObject::draw(
  ...
  _shaderMaterial.getShaderStrategy()->SetUniforms(
      _shaderMaterial, getTransform().get_world_matrix(), viewMatrix,
      projectionMatrix, lights);
...
const Vao& RenderableObject::getVao() const { return _vao; }

// inherite from ShaderMaterialable
std::shared_ptr<const ShaderStrategy> RenderableObject::getShaderStrategy()
    const {
  return _shaderMaterial.getShaderStrategy();
}

void RenderableObject::setShaderStrategy(
