...
/*
    ...
    if(!mtlStream.bad())
    {
        ...
        while (!mtlStream.eof())
        {
            ...
            else if(line[0] == 'n' && line[1] == 'e' && line[2] == 'w' && line[3] == 'm' && line[4] == 't' && line[5] == 'l' && nbMaterials == 0)
            {
                sscanf(line, "newmtl %s", name);
                nbMaterials++;
            }
    ...
    if (!objStream.bad())
    {
        ...
        while (!objStream.eof())
        {
            ...
            if ((line[0] == 'o' || line[0] == 'g') && nbMesh != 0)
            {
                ...
                std::cout << "Number of vertex normals: " << normalList.size() << std::endl;
                std::cout << "Number of Map coordinates: " << texCoordList.size() << std::endl;
                std::cout << "Number of triangular faces: " << faceIndexes.size() / 3 << std::endl;

                if (!duplicatedNormals.empty() && duplicatedVertices.size() != duplicatedNormals.size())
                {
                    std::cout << "/!\\ Error while loading normals" << std::endl;
                }

                if (!duplicatedTexCoords.empty() && duplicatedVertices.size() != duplicatedTexCoords.size())
                {
