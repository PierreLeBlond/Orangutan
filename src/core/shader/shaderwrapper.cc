...
void ShaderWrapper::StoreUniformLocation() {
  ...
  GL_CHECK_ERROR(glGetProgramInterfaceiv(shader_program_.GetId(), GL_UNIFORM,
  ...
  for (int i = 0; i < nbOfUniforms; i++) {
    GLint results[4];
    GL_CHECK_ERROR(glGetProgramResourceiv(shader_program_.GetId(), GL_UNIFORM,
                                          i, 4, properties, 4, NULL, results));
    if (results[3] != -1) {
    }
    GLint nameBufSize = results[0] + 1;
    char *name = new char[nameBufSize];
    GL_CHECK_ERROR(glGetProgramResourceName(shader_program_.GetId(), GL_UNIFORM,
                                            i, nameBufSize, NULL, name));
...
void ShaderWrapper::StoreAttributeLocation() {
  ...
  GL_CHECK_ERROR(glGetProgramInterfaceiv(shader_program_.GetId(),
                                         GL_PROGRAM_INPUT, GL_ACTIVE_RESOURCES,
                                         &nbOfAttributes));
  ...
  for (int i = 0; i < nbOfAttributes; i++) {
    GLint results[3];
    GL_CHECK_ERROR(glGetProgramResourceiv(shader_program_.GetId(),
                                          GL_PROGRAM_INPUT, i, 3, properties, 3,
                                          NULL, results));
    GLint nameBufSize = results[0] + 1;
    char *name = new char[nameBufSize];
    GL_CHECK_ERROR(glGetProgramResourceName(
        shader_program_.GetId(), GL_PROGRAM_INPUT, i, nameBufSize, NULL, name));
...
void ShaderWrapper::BindUniform(const std::string &name, const glm::vec3 &v) {
  ...
  GL_CHECK_ERROR(glUniform3fv(uniform_locations_[name], 1, glm::value_ptr(v)));
}
...
void ShaderWrapper::BindUniform(const std::string &name, const glm::vec4 &v) {
  ...
  GL_CHECK_ERROR(glUniform4fv(uniform_locations_[name], 1, glm::value_ptr(v)));
}
...
void ShaderWrapper::BindUniform(const std::string &name, const glm::mat3 &m) {
  ...
  GL_CHECK_ERROR(glUniformMatrix3fv(uniform_locations_[name], 1, GL_FALSE,
                                    glm::value_ptr(m)));
}
...
void ShaderWrapper::BindUniform(const std::string &name, const glm::mat4 &m) {
  ...
  GL_CHECK_ERROR(glUniformMatrix4fv(uniform_locations_[name], 1, GL_FALSE,
                                    glm::value_ptr(m)));
}
...
void ShaderWrapper::BindUniform(const std::string &name, float val) {
  ...
  GL_CHECK_ERROR(glUniform1f(uniform_locations_[name], val));
}
...
void ShaderWrapper::BindUniform(const std::string &name, int val) {
  ...
  GL_CHECK_ERROR(glUniform1i(uniform_locations_[name], val));
}
...
void ShaderWrapper::BindUniform(const std::string &name, unsigned int val) {
  ...
  GL_CHECK_ERROR(glUniform1ui(uniform_locations_[name], val));
}
...
void ShaderWrapper::BindTexture(const std::string &name, unsigned int id) {
  ...
  GL_CHECK_ERROR(glActiveTexture(GL_TEXTURE0 + texture_bindings_[name]));
