...
bool ShaderProgram::attach(unsigned int id)
{
    ...
    if(success = (numberOfAttachedShader < MAX_ATTACHED_SHADER))
...
{
    return build(_vertexShader.getPath(), _fragmentShader.getPath(), _geometryShader.getPath());
}
                          ...
                          const std::string& geometryShaderPath)
{
    ...
    glGetAttachedShaders(_handle.getId(), MAX_ATTACHED_SHADER, &numberOfAttachedShader, shaders);
    for(int i = 0; i < numberOfAttachedShader; ++i)
        detach(shaders[i]);

    bool success = true;

    if (success = (_vertexShader.compile(vertexShaderPath)
                && _fragmentShader.compile(fragmentShaderPath)))
    {
        attach(_vertexShader.getId());
        attach(_fragmentShader.getId());

        if(success = (geometryShaderPath == ""
